...
2. `.github/workflows/deploy.yml` — two manual-dispatch jobs (one per domain) using
   SamKirkland/FTP-Deploy-Action, uploading `dist/<domain>/` with an exclude list
   (`site/content`, `site/media`, `site/data`, `site/cache`, `site/config.local.php`), gated on
   secrets `FTP_HOST_VIAJE` / `FTP_USER_VIAJE` / `FTP_PASS_VIAJE` and the `_TTDP` equivalents.
3. `docs/cutover-runbook.md` — numbered steps for Anton: Hostinger website creation per domain,
   ...
   uploads). Include the pre-cutover check of thingstodoinparaguay.com's existing sitemap (plan §1).
...
paragraph to the director session `session_01Lch1f62XQuwvsDniCpbkGq`; otherwise skip.
